...
EnterRod() 
{
    RodGui := Gui("+AlwaysOnTop", "Enter Rod Location")
    RodGui.Add("Text", , "Enter rod location dune rod then normal rod:")
    RodGui.Add("Text", , "Format: dune rod, normal rod")
    LocationInput := RodGui.Add("Edit", "w300 vCoordInput")
    RodGui.Add("Button", "w100", "Submit").OnEvent("Click", (*) => ChangeRodLocation(RodGui, LocationInput))
    RodGui.Show("AutoSize Center")
}

ChangeRodLocation(EnterRodGui, RodInput) {
    global DuneRodLocation, NormalRodLocation

    coords := RodInput.Value
    values := StrSplit(coords, ",")

    if (values.Length != 2) {
        MsgBox "Invalid input. Please enter 1 comma-separated numbers (Dune Rod, Normal rod)."
        return
    }

    DuneRodLocation := values[1], NormalRodLocation := values[2]

    EnterRodGui.Destroy()
    ToolTip("Positions Loaded. Press q to start fishing.", 1200, 900)
}
